...
<div class="card card-outline card-dark shadow rounded0-0">
    <div class="card-header">
        <h3 class="card-title"><b>قائمة الطلبات</b></h3>
        <div class="card-tools">
            <button class="btn btn-primary btn-flat btn-sm" type="button" id="update_status"><i class="fa fa-edit"></i> تحديث الحالة</button>
            <button class="btn btn-danger btn-flat btn-sm" type="button" id="delete_order"><i class="fa fa-trash"></i> حذف</button>
            <a class="btn btn-default btn-flat border btn-sm" href="./?page=orders"><i class="fa fa-angle-left"></i> رجوع للقائمة</a>
        </div>
    </div>
    <div class="card-body">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-6">
                    <label for="" class="text-muted">رقم الطلب</label>
                    <div class="ml-3"><b><?= isset($ref_code) ? $ref_code : "N/A" ?></b></div>
                </div>
                <div class="col-md-6">
                    <label for="" class="text-muted">تاريخ الطلب</label>
                    <div class="ml-3"><b><?= isset($date_created) ? date("M d, Y h:i A", strtotime($date_created)) : "N/A" ?></b></div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <label for="" class="text-muted">الحالة</label>
                    <div class="ml-3">
                        <?php if(isset($status)): ?>
                            <?php if($status == 0): ?>
                                <span class="badge badge-secondary px-3 rounded-pill">انتظار</span>
                            <?php elseif($status == 1): ?>
                                <span class="badge badge-primary px-3 rounded-pill">قيد التجهيز</span>
                            <?php elseif($status == 2): ?>
                                <span class="badge badge-success px-3 rounded-pill">جاهز للتوصيل</span>
                            <?php elseif($status == 3): ?>
                                <span class="badge badge-warning px-3 rounded-pill">على الطريق</span>
                            <?php elseif($status == 4): ?>
                                <span class="badge badge-default bg-gradient-teal px-3 rounded-pill">تم التوصيل</span>
                            <?php else: ?>
                                <span class="badge badge-danger px-3 rounded-pill">تم الالغاء</span>
                            <?php endif; ?>
                        <?php else: ?>
                            N/A
                        <?php endif; ?>
                ...
                <div class="w-100" id="order-list">
                        ...
                        <div class="d-flex align-items-center w-100 border justify-content-center">
                            <div class="col-12 flex-grow-1 flex-shrink-1 px-1 py-1">
                                <small class="text-muted">لا نتائج</small>
                            </div>
                        </div>
                        <?php endif; ?>
                        <div class="d-flex align-items-center w-100 border">
                            <div class="col-auto flex-grow-1 flex-shrink-1 px-1 py-1">
                                    <h3 class="text-center">الاجمالي</h3>
                            </div>
                            <div class="col-auto text-right">
...
<script>
    $(function(){
        $('#update_status').click(function(){
            uni_modal("تحديث حالة الطلب","orders/update_status.php?id=<?= isset($id) ? $id :'' ?>")
        })
        $('#btn-cancel').click(function(){
            _conf("هل أنت متأكد من إلغاء هذا الطلب؟","cancel_order",[])
        })
        $('#delete_order').click(function(){
            _conf("هل انت متأكد من حذف الطلب","delete_order",[])
        })
    })
