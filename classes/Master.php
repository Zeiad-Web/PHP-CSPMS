...
Class Master extends DBConnection {
	...
	function save_category(){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"تم تسجيل الفئة بنجاح");
			else
				$this->settings->set_flashdata('success',"تم تحديث الفئة بنجاح");
		}else{
			$resp['status'] = 'failed';
	...
	function delete_category(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"تم حذف الفئة بنجاح");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_brand(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "الوكالة موجودة بالفعل";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
		...
		if($save){
			...
			if(empty($id))
				$resp['msg'] = "تم تسجيل الوكالة بنجاح";
			else
				$resp['msg'] = "تم تحديث الوكالة بنجاح";
	...
	function delete_brand(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"تم حذف الوكالة بنجاح");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_product(){
		...
		if($save){
			...
			$resp['id'] = $pid ;
			if(empty($id))
				$resp['msg'] = "تم تسجيل المنتج بنجاح";
			else
				$resp['msg'] = "تم تحديث المنتج بنجاح";
			if(!empty($_FILES['img']['tmp_name'])){
				$ext = $ext = pathinfo($_FILES['img']['name'], PATHINFO_EXTENSION);
	...
	function delete_product(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"تم حذف المنتج بنجاح");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_service(){
		...
		if($save){
			...
			$resp['id'] = $pid ;
			if(empty($id))
				$this->settings->set_flashdata('success',"تم تسجيل الخدمة بنجاح");
			else
				$this->settings->set_flashdata('success',"تم تحديث الخدمة بنجاح");
				if(!empty($_FILES['img']['tmp_name'])){
					$ext = $ext = pathinfo($_FILES['img']['name'], PATHINFO_EXTENSION);
					$dir = base_app."uploads/serv/";
					if(!is_dir($dir))
					mkdir($dir);
					$name = $pid.".".$ext;
					if(is_file($dir.$name))
					...
					$move = move_uploaded_file($_FILES['img']['tmp_name'],$dir.$name);
					if($move){
						$this->conn->query("UPDATE `service_list` set image_path = CONCAT('uploads/serv/$name','?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '{$pid}'");
					}else{
						$resp['msg'] .= " فشل تحميل الصورة";
					}
				}
		}else{
			$resp['status'] = 'failed';
	...
	function delete_service(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"تم حذف الخدمة بنجاح");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_stock(){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"تم اضافة الكمية بنجاح");
			else
				$this->settings->set_flashdata('success',"تم تعديل الكمية بنجاح");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	function delete_stock(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"تم حذف الكمية بنجاح");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_mechanic(){
		...
		if($save){
			$resp['status'] = 'success';
			if(empty($id))
				$this->settings->set_flashdata('success',"تم اضافة المختص بنجاح.");
			else
				$this->settings->set_flashdata('success',"تم تحديث المختص بنجاح.");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error."[{$sql}]";
		}
	...
	function delete_mechanic(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"تم حذف المختص بنجاح");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_request(){
		...
		if($save){
			...
			if($save){
				...
				$resp['id'] = $rid;
				if(empty($id))
				$resp['msg'] = " تم تقديم طلب الخدمة بنجاح";
				else
				$resp['msg'] = "  تم تحديث تفاصيل طلب الخدمة بنجاح";
			}else{
				$resp['status'] = 'failed';
				$resp['error'] = $this->conn->error;
				$resp['sql'] = $sql;
				if(empty($id))
				$resp['msg'] = " فشل ارسال طلب الخدمة";
				else
				$resp['msg'] = " فشل تحديث تفاصيل طلب الخدمة.";
			...
			$resp['sql'] = $sql;
			if(empty($id))
			$resp['msg'] = "الرجاء تسجيل الدخول اولا";
			else
			$resp['msg'] = " فشل تحديث طلب الخدمة";
		}
	...
	function delete_request(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"تم حذف الطلب بنجاح");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function save_to_cart(){
		...
		if($save){
			...
			$resp['status'] = 'failed';
			$resp['msg'] = " فشل اضافة المنتج الى السلة";
			$resp['error'] = $this->conn->error;
		}
		return json_encode($resp);
	}
	function update_cart_quantity(){
		...
		if($available < 1){
			$resp['status'] = 'failed';
			$resp['msg'] = " المنتج لايحتوي على مخزون متاح.";
			...
			$resp['msg'] = " يتوفر المنتج في المخزون [{$available}] فقط";
		}else{
			$resp['status'] = 'success';
	...
	function remove_from_cart(){
		...
		if($del){
			$resp['status'] = 'success';
			$resp['msg'] = " تم إزالة المنتج من قائمة عربة التسوق بنجاح.";
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = " فشل إزالة المنتج من قائمة عربة التسوق.";
			$resp['error'] = $this->conn->error;
		}
		if($resp['status'] == 'success')
			$this->settings->set_flashdata('success',$resp['msg']);
		...
		if($save){
			...
			if(!empty($data)){
				...
				if($save2){
					...
					$resp['msg'] = " تم تقديم الطلب بنجاح.";
				}else{
					$resp['status'] = 'failed';
					$resp['msg'] = " لقد فشل الطلب في التنفيذ.";
					$resp['error'] = $this->conn->error;
	...
	function cancel_order(){
		...
		if($update){
			$resp['status'] = 'success';
			$resp['msg'] = "تم إلغاء الطلب.";
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = " فشل إلغاء الطلب.";
			$resp['error'] = $this->conn->error;
		}
		if($resp['status'] == 'success')
	...
	function cancel_service(){
		...
		if($update){
			$resp['status'] = 'success';
			$resp['msg'] = " تم إلغاء طلب الخدمة.";
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = " فشل إلغاء طلب الخدمة.";
			$resp['error'] = $this->conn->error;
		}
		if($resp['status'] == 'success')
		$this->settings->set_flashdata('success',$resp['status']);
	...
	function update_order_status(){
		...
		if($update){
			$resp['status'] ='success';
			$resp['msg'] = " تم تحديث حالة الطلب بنجاح.";
		}else{
			$resp['error'] = $this->conn->error;
			$resp['status'] ='failed';
			$resp['msg'] = " فشلت حالة الطلب في التحديث.";
		}
		if($resp['status'] == 'success')
		$this->settings->set_flashdata('success',$resp['msg']);
		return json_encode($resp);
	...
	function delete_order(){
		...
		if($delete){
			$resp['status'] ='success';
			$resp['msg'] = " تم حذف حالة الطلب بنجاح.";
		}else{
			$resp['error'] = $this->conn->error;
			$resp['status'] ='failed';
			$resp['msg'] = " فشلت حالة الطلب في الحذف.";
		}
		if($resp['status'] == 'success')
		$this->settings->set_flashdata('success',$resp['msg']);
		return json_encode($resp);
