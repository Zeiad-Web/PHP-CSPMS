...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(empty($id)){
			...
			if($qry){
				$this->settings->set_flashdata('success','تم تحديث البيانات بنجاح');
				foreach($_POST as $k => $v){
					if($k != 'id'){
						if(!empty($data)) $data .=" , ";
	...
	public function delete_users(){
		...
		if($qry){
			$this->settings->set_flashdata('success','تم حذف تفاصيل المستخدم بنجاح.');
	...
	public function save_client(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = ' البريد الإلكتروني موجود بالفعل.';
		}else{
			if(empty($id)){
			...
			if($save){
				...
				if(empty($id)){
					...
					$resp['msg'] = " تم تحديث تفاصيل حساب العميل بنجاح.";
				}
			}else{
				$resp['status'] = 'failed';
				if(empty($id)){
					$resp['msg'] = " فشل تسجيل الحساب لسبب ما.";
				}else if($this->settings->userdata('id') == $id && $this->settings->userdata('login_type') == 2){
					$resp['msg'] = " فشل تحديث تفاصيل الحساب.";
				}else{
					$resp['msg'] = " فشل تحديث تفاصيل حساب العميل.";
				}
			}
		}
	...
	function delete_client(){
		...
		if($del){
			$resp['status'] = 'success';
			$resp['msg'] = ' تم حذف حساب العميل بنجاح.';
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = " فشل حذف حساب العميل";
		}
		if($resp['status'] =='success')
		$this->settings->set_flashdata('success',$resp['msg']);
		return json_encode($resp);
